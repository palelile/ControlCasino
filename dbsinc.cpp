...
bool dbSinc::conectarRemota(QString servidor, QString puerto, QString base, QString usuario, QString contr)
{
	...
	return ((dbRemota *)db_remota)->conectar(servidor,puerto, base, usuario, contr);
}

bool dbSinc::conectarLocal(QString archivo)
{
	db_local = new dbLocal();
	return ((dbLocal *)db_local)->conectar(archivo);
}

void dbSinc::desconectar()
{
	db_local->desconectar();
...
QStringList * dbSinc::TablaTemporal(db *base_datos, QString tabla, QString campos, int ultimo_id)
{
	if ( !base_datos->isOpen() )
		return NULL;
	QSqlQueryModel * modelo = modeloConsulta(NULL, "SELECT "+campos+
											 "FROM "+tabla+
...
bool dbSinc::PersistirTablaTemporal(db *base_datos, QString tabla, QString campos, QStringList *listas)
{
	...
	for ( int i = 0; i < listas->count(); i++)
	{
		...
		if ( (i % 200 == 0) || (i == listas->count()-1) )
		{
			...
			if ( !base_datos->agregar(tabla, campos, valores) )
				return false;
			valores = "(";
		}
	}
...
int dbSinc::ultimoRegistro(db *base_datos, QString tabla)
{
	...
	if ( base_datos == db_local )
		salida = base_datos->buscar("ultimoregistrolocal", "indices", cond);
	else
		salida = base_datos->buscar("ultimoregistroremoto", "indices", cond);
	if (salida)
		salida->at(0).toInt();
	return -1;
}
...
{
	QStringList *salida = base_datos->buscar("MAX(id)", tabla, "");
	if (salida)
...
bool dbSinc::guardarUltimoRegistro(QString tabla, int ultimo_registro_local, int ultimo_registro_remoto)
{
	...
	QString valores = "'"+tabla+"','"+ultimo_registro_local+"','"+ultimo_registro_remoto+"'";
